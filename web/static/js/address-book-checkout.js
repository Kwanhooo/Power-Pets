...
$(function () {
    (function () {
        $('#address-book-checkout-btn').click(function () {
...
function saveChanges(index) {
    ...
    $("#consignee-checkout").val(consignee);
    $("#city-checkout").val(city);
    $("#address-checkout").val(address);
    $("#contact-checkout").val(contact);
...
function setupDisplayTabs(data) {
    ...
    for (let i = 0; i < addressArray.length - 1; i++) {
        ...
        new_display_div.append($("" +
            ...
            "                    style=\"max-width: 90%;margin-top: 10px;\" onclick=\"saveChanges(" + (i) + ");\">\n" +
            "                就用这个啦\n" +
            "            </button>\n" +
            "        </div>\n" +
            "    </div>\n" +
